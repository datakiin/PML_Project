...
This project attempts to predict whether subjects have performed a physical exercise correctly using Human Activity Recogniton (HAR). The training and test datasets are processed by removing columns that are not useful to train our models. We consider multiple approaches and select a random forest algorithm that yields an accuracy of 99% on the test set. As a final validation, the selected model is used on 20 tests based on previously unseen data.
...
model <- train(classe ~ ., data = training, method = 'rf', trControl =
...
results <- "./results"
if(!file.exists("./data/results")) {file.create(results)}
