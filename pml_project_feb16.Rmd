...
The data for this project is provided by the [HAR]{http://www.google.com.pk}. The participants in the study were asked to perform bicep curls correctly and incorrectly in 5 different ways.  Each way is considered a class, A-F, and recorded in the classe variable of the data set.
...
The sensor data was recorded by accelerometers on the participant's belt, forearm, arm, and dumbell.
...
Building random forest classifications for a large number of variables is a computationally intensive task and can take some time. We use the doParallel package in R to enable parallel computations and speed up the process considerably on most personal computers processing one computation at a time.
...
The out of sample error is the error rate observed on the preTest data which is 100% - accuracy = 100% - 99.68% = .32%.
